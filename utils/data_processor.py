...
class DataProcessor:
    ...
    def prepare_data(self,
        ...
        if wavelength is None:
            wavelength = np.full_like(lat, 0.5) 
        else:
            wavelength = np.asarray(wavelength)

       
        time_nd = time / self.time_scale
        wavelength_nd = wavelength / 0.5  
    ...
    def generate_training_data(self, n_samples=1000):
        ...
        n_night = int(n_samples * 0.3) 
        n_transition = int(n_samples * 0.2) 
        ...
        transition_hours = np.concatenate([
            np.random.uniform(5, 7, n_transition // 2),  
            np.random.uniform(17, 19, n_transition // 2)  
        ])
